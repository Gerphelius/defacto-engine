...
static void AssaultListItem(DF::GameMemory* gameMemory,
                            Assault* assault,
    ...
    })
    {
        ...
        })
        {
            ...
            })
            {
                Text("SUPPRESSION:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", assault->suppression), 12);
            }
        }

        Spacer();
        Button("ASSIGN", callback, data);
    }
}

static void ScientistListItem(DF::GameMemory* gameMemory,
                              Scientist* scientist,
                              void (*callback)(Clay_ElementId, Clay_PointerData, void*),
                              void* data)
{
    ...
    })
    {
        ...
        })
        {
            ...
            })
            {
                Text("PHYSICS:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", scientist->physics), 12);
            }
            CLAY_AUTO_ID({
                .layout = {
                    .sizing = { .width = CLAY_SIZING_GROW(0), .height = CLAY_SIZING_FIXED(0) },
            ...
            })
            {
                Text("MECHANICS:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", scientist->mechanics), 12);
            }
            CLAY_AUTO_ID({
                .layout = {
                    .sizing = { .width = CLAY_SIZING_GROW(0), .height = CLAY_SIZING_FIXED(0) },
            ...
            })
            {
                Text("BIOLOGY:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", scientist->biology), 12);
            }
            CLAY_AUTO_ID({
                .layout = {
                    .sizing = { .width = CLAY_SIZING_GROW(0), .height = CLAY_SIZING_FIXED(0) },
            ...
            })
            {
                Text("CHEMISTRY:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", scientist->chemistry), 12);
            }
            CLAY_AUTO_ID({
                .layout = {
                    .sizing = { .width = CLAY_SIZING_GROW(0), .height = CLAY_SIZING_FIXED(0) },
            ...
            })
            {
                Text("ELECTRONICS:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", scientist->electronics), 12);
            }
        }
...
static void ReconListItem(DF::GameMemory* gameMemory,
                          Recon* recon,
                          void (*callback)(Clay_ElementId, Clay_PointerData, void*),
                          void* data)
{
    ...
    })
    {
        ...
        })
        {
            ...
            })
            {
                Text("DETECTION:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", recon->detection), 12);
            }
            CLAY_AUTO_ID({
                .layout = {
                    .sizing = { .width = CLAY_SIZING_GROW(0), .height = CLAY_SIZING_FIXED(0) },
            ...
            })
            {
                Text("STEALTH:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", recon->stealth), 12);
            }
        }
...
static void SupportListItem(DF::GameMemory* gameMemory,
                            Support* support,
                            void (*callback)(Clay_ElementId, Clay_PointerData, void*),
                            void* data)
{
    ...
    })
    {
        ...
        })
        {
            ...
            })
            {
                Text("MEDICAL:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", support->medical), 12);
            }
            CLAY_AUTO_ID({
                .layout = {
                    .sizing = { .width = CLAY_SIZING_GROW(0), .height = CLAY_SIZING_FIXED(0) },
            ...
            })
            {
                Text("LOGISTICS:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", support->logistics), 12);
            }
            CLAY_AUTO_ID({
                .layout = {
                    .sizing = { .width = CLAY_SIZING_GROW(0), .height = CLAY_SIZING_FIXED(0) },
            ...
            })
            {
                Text("SECRECY:", 12);
                Spacer();
                Text(DF::StrFormat(&gameMemory->transient, "%.2f", support->secrecy), 12);
            }
        }
...
//         Agent last =
//           agentData->gameState->agentsForHire.list[agentData->gameState->agentsForHire.count -
//           1];
//
...
static void CreateAgentsUI(DF::GameMemory* gameMemory)
{
    ...
    //            {
    //                .layout = {
    //                    .sizing = { .width = CLAY_SIZING_PERCENT(0.5), .height =
    //                    CLAY_SIZING_GROW(0) }, .padding         = CLAY_PADDING_ALL(16), .childGap
    //                    = 16, .layoutDirection = CLAY_TOP_TO_BOTTOM,
    //                },
